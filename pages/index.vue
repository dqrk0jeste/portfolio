...
<template>
  <div id="background" class="transition-colors duration-700 z-0" :class="currentColor">
    <BackgroundCircle />
    <div v-if="count > 1 && count < 2 + projectCount" class="hidden lg:block fixed w-full h-1 bottom-16 left-0 bg-pink-400 transition-all duration-500 z-[1]"></div>
    <div class="relative transition-all duration-700 z-10" :style="`transform: translateY(-${ pages[count]?.dontShift ? (count - 1) * 100 : count * 100 }dvh)`">
      <Landing />
      <About />
      <section id="projects">
        <Project
